...
type TelepresenceMetadata struct {
	telepresenceMetadata metadata.MD
}

var telepresenceHeaders TelepresenceMetadata

func propogateContextHttp(r *http.Request) context.Context {
	ctx := r.Context()
	...
	if telHeader != "" {
		...
		fmt.Printf("Added %v to context.", telepresenceHeaders.telepresenceMetadata)
	}
